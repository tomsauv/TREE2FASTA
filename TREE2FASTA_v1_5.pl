...
if ($first_fasta_line=~/^(.).+/ && $1 ne '>'){
    print "\n","Abort! Check example fasta file format or check file order in command line","\n";
    exit;
}
...
if ($first_tree_line=~/(.+)/ && $1 ne '#NEXUS'){
    print "\n","Abort! Check example tree file format or check file order in command line","\n";
    exit;
}
...
if ($j==$c){
	print "\n","However, none of the tree labels contains color or name edits! Abort!","\n";
	print "\n","Did you click on figtree's TAXA button to edit labels","\n";
	print "(TREE2FASTA cannot parse edits made with NODES or CLADES buttons)","\n";
	print "\n","Did you save the edited tree file with figtree's SAVE AS function?","\n";
	print "(TREE2FASTA cannot parse files produced with the EXPORT TREES function)","\n";
	exit;
}
#xxxxxx
